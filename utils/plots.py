...
def plot_one_box(x, img, color=None, label=None, line_thickness=3):
    ...
    lat_begin = 36.334799
    lat_end = 36.334900
    lat = random.uniform(lat_begin, lat_end)
    # lon = 116.499527
    # lon_begin = 116.499510
    # lon_end = 116.499550
    lon = 116.446542
    lon_begin = 116.446500
    lon_end = 116.446611
    lon = random.uniform(lon_begin, lon_end)
    # alti = 40.310768
    # alti_begin = 40.310750
    # alti_end = 40.310798
    alti = 38.310768
    alti_begin = 38.310750
    alti_end = 38.310798
    alti = random.uniform(alti_begin, alti_end)
    if label:
        ...
        loc = "lat: " + \
            f'{lat:.6f}' + ", lon: " + f'{lon:.6f}' + \
            ", alti: " + f'{alti:.6f}'
        cv2.putText(img, label, (tl_x, tl_y-10), 0, tl / 3,
                    color, thickness=tf, lineType=cv2.LINE_AA)
        cv2.putText(img, loc, (tl_x, tl_y-1), 0, tl / 3,
                    color, thickness=tf, lineType=cv2.LINE_AA)
